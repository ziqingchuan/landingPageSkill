...
- 本文件给出的页面结构、模块方向和文案建议是"内容方向"，不是"必现模块清单"。只有当内容有依据、且服务于转化目标时才使用对应模块；不得为了凑结构而堆模块（与 `business-rules.md` 第 13 节一致）。
...
**结构即信息。** 编号、眉标、分隔线、标签应承载关于内容的真实含义，而不是装饰。只有当内容本身就是有序流程或时间线时，才使用 01/02/03 这类编号标记；用之前先确认它是否真的必要。当内容适合序列化呈现（如学习阶段、课程体系、选课流程），**罗马数字 I / II / III** 可作为视觉节奏标记，比阿拉伯数字更有编辑感和仪式感，但同样要求内容本身确实有序列逻辑——不要为了装饰而使用。
...
**内容密度由信息需求决定。** 教育类落地页不必须"简洁到极致"。当目标受众需要充分信息才能做出决策（如课程体系选择、多科目对比、分龄适配），高内容密度是合理的。允许信息量较大的长页面，前提是：信息组织有序（通过标题层级、分隔、锚点导航实现可扫描性），视觉层级清晰，每段内容都有存在的理由。不要为了"极简"而删掉家长真正需要的决策信息，也不要为了"丰富"而堆砌与转化无关的内容。
...
- **装饰性元素**（图标、背景图形、装饰性插画）：优先使用内联 SVG，确保单文件自包含、无限缩放，不依赖外部资源。
- **照片级素材**（场景图、人物图、教学环境）：使用免费 CDN 图库直链（如 Unsplash、Pexels），选用与页面主题、科目、受众匹配的高质量图片，确保视觉品质和真实感。
- **技术约束**：不使用本地文件路径；图片必须添加明确的尺寸约束（`width`/`height` 属性或 CSS 尺寸），避免布局抖动；移动端图片需适配屏幕宽度。
...
- **斜体关键词** → 叙事节奏——在标题或副标题中对核心概念做轻量提示，引导视线而不打断阅读流。例：*"让每个孩子*找到*属于自己的中文表达"*。

三种方案可以组合使用，但同一行内不宜超过两种。斜体不宜用于长段落正文——中文斜体可读性较低，控制在标题、引用或短句内使用。
...
> 模块取舍原则：师资、评价、成果、品牌背书、价格、FAQ、学习路径、流程说明和价值列表都不是无条件必备模块。只有当前页面方案确实需要、且内容有依据时才使用。不得为了凑满上述结构而堆砌空模块。模块数量、顺序、篇幅由 AI 根据实际配置和内容自主决定（与 `business-rules.md` 第 13 节一致）。
...
- 必须包含至少两类信任信号。
- 信任信号不限于评价和对比表，应根据页面内容选用最合适的形态。以下是常见的信任信号形态：
  - **数据指标横条**：一组横向排列的核心数字（学员数、完课率、续费率等），适合首屏或信任模块顶部，快速传递规模感。
  - **师资卡片**：教师头像 + 姓名 + 资质标签 + 一句话教学理念，比纯文字更可信。
  - **奖项徽标网格**：2-4 列徽标矩阵（教育奖项、行业认证、合作机构 logo），用于品牌背书，简洁且不占大量篇幅。
  - **评分徽章**：星级评分 + 来源平台标识（如 Trustpilot、Google Review），适合放在评价模块旁或首屏角落。
  - **适配筛选标签**：显示"适合 X-Y 岁零基础 / 有基础 / 考试导向"等标签，帮助家长快速判断适配性。
  - **家长评价与学员成果**：引用式卡片，带头像、姓名首字母或地区标识，内容具体到结果。
  - **课程体系可视化**：级别、阶段、能力目标的结构化展示，传递教研专业性。
  - **合作机构或媒体背书**：logo 墙或引用语。
...
- 建议在首屏、价值主张之后、页面底部等关键位置提供 CTA 入口；但具体数量和位置由页面方案自主决定，不要机械地在每个模块重复 CTA（与 `business-rules.md` 第 6、13 节一致）。
...
- 支持**主 CTA + 副 reassurance 文案**的双行 CTA 模式：按钮上方或下方配一行小字补充信息（如"无需信用卡 · 15 分钟 · 随时取消"或"已有 12,000+ 家庭体验"），降低决策阻力。主按钮保持醒目，副文案用较轻的字重和较小的字号，两者在视觉上形成主次关系，不要竞争注意力。

## 13. 表单规范

- 联系方式按 `contactMode` 收集，确保至少一条可联系途径。
- 字段适用性由配置和科目条件规则决定：中文课程年龄和水平必须出现，数学或英文课程年级必须出现（详见 `user-config-spec.md` 第五节、`business-rules.md` 第 7.2 节）。
...
**第一阶段：brainstorm。** 基于当前科目、受众、投放场景、主题以及 `project.notes` 中的用户风格偏好（如有），形成一个紧凑的设计计划：
