...
| `required` | `boolean` | 是 | 是否必答（针对进线科目的首次流程）。 |
...
- `leadQuestions` 数组中配置的是**进线科目**的问题。
- **`leadQuestions` 只在首次进线流程中展示**（即用户第一次进入页面、为第一个孩子填写进线科目时）。后续拓科或拓娃的追加流程中，**不展示也不校验** `leadQuestions`，只填写必要的表单字段（孩子称呼、时区、年龄/水平/年级等）。
...
- 拓科科目不使用 `leadQuestions` 配置，也不使用标准题库中的问题。
- **后续拓科流程中完全不展示 `leadQuestions`**，直接跳过到资料补充阶段。
